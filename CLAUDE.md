...
- **Logging**: Use `tracing` crate, not `println!` / `dbg!`. For container/OTel support, build with `--features otel` (`OTEL_EXPORTER_OTLP_ENDPOINT` is pre-configured in `mise.toml`).
